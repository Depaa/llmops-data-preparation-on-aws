...
AmazonS3_node1716105954683 = glueContext.create_dynamic_frame.from_options(
  ...
  connection_type='s3', 
  format='json', 
  connection_options={'paths': [args['SOURCE']], 'recurse': True}, 
...
detection_parameters = {
  'EMAIL': [{
    'action': 'REDACT',
    'actionOptions': {'redactText': '[EMAIL]'}
  }],
  'CREDIT_CARD': [{
    'action': 'REDACT',
    'actionOptions': {'redactText': '[CREDIT_CARD]'}
  }]
}

entity_detector = EntityDetector()
DetectSensitiveData_node1716106115991 = entity_detector.detect(
  AmazonS3_node1716105954683, 
  detection_parameters, 
  'DetectedEntities', 
  'HIGH'
)
...
AmazonS3_node1716106268819 = glueContext.write_dynamic_frame.from_options(
  frame=DetectSensitiveData_node1716106115991, 
  connection_type='s3', 
  format='json', 
  connection_options={'path': [args['DESTINATION']], 'partitionKeys': []}, 
